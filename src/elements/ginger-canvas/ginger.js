var Ginger = function (root) {
  ...
  var slider = root.getElementById('range');
  var selected = 'eyes';
  ...
  var morphs = {
    eyes: {
      ...
      behavior: function (value) {
        ...
        var recede = linear(sex, 0, -0.125, 1);
    ...
    for (var item in morphs) {
      ...
      for (var j = 0; j < morphTarget.targets.length; j++) {
        ...
        if (morphTarget.targets[j] !== morphTarget.targets[target]) {
          ...
          morphTarget.mesh.mesh.morphTargetInfluences[index] = Math.abs(
            morphTarget.value
          );
        }
      }
  ...
  function onsharepress(event) {
    var modal = root.getElementById('share-modal');
    modal.classList.remove('hidden');

    var shareLink = root.getElementById('share-link');
  ...
  function onscreenshotpress(event) {
    var counter = root.getElementById('counter');
  ...
  function onmousetrack(event) {
    ...
    var elButton = root.getElementById('mousetrack');
  ...
  function onscreenshotdismiss(event) {
    var modal = root.getElementById('screenshot-modal');
    modal.classList.add('hidden');
  }
  ...
  function screenshot() {
    ...
    var image = root.getElementById('screenshot-image');
    image.src = getImage;
  }
  ...
  return {
    init: function () {
      ...
      root.getElementById('renderer').appendChild(renderer.domElement);

      // Allow viewport resizing whenever the window resizes.
      window.onresize = onresize;

      // Setup event so ginger's eyes track the mouse
      var elRenderer = root.getElementById('renderer');
      ...
      root.getElementById('range').onchange = onrangeslide;
      root.getElementById('range').oninput = onrangeslide;
      root.getElementById('morph').onchange = onselect;
      root.getElementById('share').onclick = onsharepress;
      root.getElementById('mousetrack').onclick = onmousetrack;
      root.getElementById('screenshot').onclick = onscreenshotpress;
